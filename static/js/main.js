document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkWhatsappField = async (input) => {
        ...
        try {
            ...
            setFeedback('whatsapp', data.formatted ? `WhatsApp válido: ${data.formatted}` : 'WhatsApp válido.', 'success');
            return true;
        } catch (error) {
    ...
    document.querySelectorAll('[data-email-check="true"]').forEach((input) => {
        input.addEventListener('blur', () => checkEmailField(input));
    });

    document.querySelectorAll('[data-whatsapp-check="true"]').forEach((input) => {
        input.addEventListener('blur', () => checkWhatsappField(input));
    });
