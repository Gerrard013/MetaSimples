...
BILLING_CONFIG = {
    'mensal': {'months': 1, 'days': 30, 'label': '1 mês', 'discount_label': 'acesso mensal'},
    'trimestral': {'months': 3, 'days': 90, 'label': '3 meses', 'discount_label': 'pagamento antecipado'},
    'anual': {'months': 12, 'days': 365, 'label': '1 ano', 'discount_label': 'melhor economia'},
}

DEFAULT_PRICES = {
    # Preço coerente: trimestral/anual antecipados têm desconto real, mas não ficam iguais ao mensal.
    'controle': {
        'mensal': Decimal('150.00'),
        'trimestral': Decimal('299.70'),  # R$ 99,90/mês
        'anual': Decimal('958.80'),       # R$ 79,90/mês
    },
    'metasimples': {
        'mensal': Decimal('150.00'),
        'trimestral': Decimal('299.70'),
        'anual': Decimal('958.80'),
    },
}
...
def mp_customer_total(base_amount: Decimal) -> Decimal:
    """Calcula valor cobrado do cliente com taxa configurável.

    Mercado Pago pode mudar taxas por método/antecipação. Por isso, as taxas ficam em env vars.
    Defaults conservadores para não deixar o G pagar a taxa sem querer.
    """
    fee_percent = Decimal(str(current_app.config.get('MERCADOPAGO_FEE_PERCENT', '5.31')).replace(',', '.')) / Decimal('100')
...
def create_mercadopago_preference(user, plan: str, billing: str = 'mensal') -> tuple[bool, str]:
    ...
    # Campos existem nas versões novas; se ainda não existirem no banco antigo, schema updater cria.
    ...
    payload = {
        ...
        'payment_methods': {
            # Aceita Pix, boleto e cartão conforme habilitado na conta Mercado Pago.
            # Não excluímos métodos. Apenas limitamos parcelas para evitar custo/risco fora do controle.
...
def approve_payment(payment: Payment, mp_payment_id: str | None = None) -> None:
    user = payment.user
    now = datetime.utcnow()
    days = int(getattr(payment, 'duration_days', None) or payment_duration_days(getattr(payment, 'billing_cycle', 'mensal')))
...
def sync_mercadopago_payment(mp_payment_id: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if response.status_code >= 400:
            current_app.logger.error('Erro ao consultar pagamento Mercado Pago %s: %s', response.status_code, response.text)
