...
class FinanceTransactionForm(FlaskForm):
    type = SelectField(
        'Tipo',
        choices=[('expense', 'Saída'), ('income', 'Entrada')],
        validators=[DataRequired()],
    )
    date = DateField('Data', validators=[DataRequired()])
    amount = StringField('Valor', validators=[DataRequired(), Length(max=40)])
    merchant = StringField('Estabelecimento / origem', validators=[Optional(), Length(max=120)])
    category = StringField('Categoria', validators=[Optional(), Length(max=80)])
    payment_method = StringField('Forma de pagamento', validators=[Optional(), Length(max=80)])
