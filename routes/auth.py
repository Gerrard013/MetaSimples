...
from services.email_service import confirm_email_token, send_welcome_email
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if not is_valid_email(email, check_deliverability=False):
            ...
            return render_template('register.html', form=form, selected_plan=selected_plan, selected_billing=selected_billing)
        ...
        existing_lead = Lead.query.filter((Lead.email == email) | (Lead.whatsapp == whatsapp)).first()
        ...
        try:
            ...
            return render_template('register.html', form=form, selected_plan=selected_plan, selected_billing=selected_billing), 500
        ...
        if sent:
            flash('Conta criada. Enviamos um link de confirmação para seu e-mail.', 'success')
        else:
        ...
        if payment_first:
            return redirect(url_for('main.payment_checkout', plan=selected_plan, billing=selected_billing))
...
def resend_confirmation():
    ...
    try:
        ...
        current_app.logger.exception(
            'Erro inesperado ao reenviar confirmação para %s',
            user.email,
        )
    ...
    if sent:
        ...
        flash(
            'Não conseguimos enviar o e-mail agora, mas sua conta continua criada. Fale com o suporte G Tech.',
            'warning',
        )
